...
outTab=data.frame()
pdf(file="vioplot.pdf", width=23, height=8,family="sans")
...
for(i in 1:ncol(rt)){
  ...
  text(x=3*(i-1)+0.5, y=mx+0.02, labels=ifelse(p<0.001, paste0("p<0.001"), paste0("p=",sprintf("%.03f",p))), cex = 1.3)
}
...
text(seq(1,(3*ncol(rt)-2),3), -0.03, xpd=NA, labels=colnames(rt), cex=1.2, srt=45, pos=2)
dev.off()
